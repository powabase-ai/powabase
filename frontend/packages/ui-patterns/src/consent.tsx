...
export const useConsentToast = () => {
  ...
  const denyAll = useCallback(() => {
    ...
    // Set on parent domain too so the cookie clears across apex/www/app subdomains.
    ...
    const parentDomain = parts.length > 1 ? `.${parts.slice(-2).join('.')}` : hostname
    const domains = [hostname, parentDomain]
