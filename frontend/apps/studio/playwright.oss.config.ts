...
// (oss-edition/docker-compose.yml, NEXT_PUBLIC_IS_PLATFORM=false).
//
...
// DASHBOARD_PASSWORD, see oss-edition/volumes/api/kong.yml), not a login page.
...
if (!username || !password) {
  throw new Error(
    'OSS_DASHBOARD_USERNAME and OSS_DASHBOARD_PASSWORD must be set to run this suite — ' +
      ...
      'oss-edition/.env under those names.'
  )
}
