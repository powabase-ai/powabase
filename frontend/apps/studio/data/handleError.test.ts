...
describe('handleError — error classification', () => {
  ...
  describe('field preservation', () => {
    ...
    it('extracts message from .error field when msg and message are absent', () => {
      const err = throwAndCatch({ error: 'Organization slug already exists', code: 409 })
      expect(err).toBeInstanceOf(UnknownAPIResponseError)
      expect(err.message).toBe('Organization slug already exists')
      expect(err.code).toBe(409)
    })

    it('message field takes priority over error field for error text', () => {
      const err = throwAndCatch({ message: 'from message field', error: 'from error field' })
      expect(err.message).toBe('from message field')
    })

    it('msg field takes priority over error field for error text', () => {
      const err = throwAndCatch({ msg: 'from msg field', error: 'from error field' })
      expect(err.message).toBe('from msg field')
    })
  })
})
