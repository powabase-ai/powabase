...
export async function getDatabasePolicies(
  ...
  // Only include schema filters when a specific schema is requested. Empty-string filter
  // values make pg-meta apply `IN ('')`, which matches no schemas and returns an empty list.
  const queryParams: Record<string, string> = {}
  if (schema) queryParams.included_schemas = schema
