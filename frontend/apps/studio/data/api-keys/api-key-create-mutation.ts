  ...
  const { data, error } = await post('/v1/projects/{ref}/api-keys', {
    ...
    body: {
      ...(payload.type === 'secret'
        ...
        // TODO: bug from upstream Supabase refactor — `name` here resolves to globalThis.name
        // (browser-default empty string, so `{...""}` is a no-op). Should be `{}` for the
        // publishable branch. Dead code while project_settings:new_api_key_format flag is off;
        // resurfaces if the flag is flipped on without also resolving issue #422.
        : name),
