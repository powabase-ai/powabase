...
client.use(
  ...
  },
  {
    ...
    async onResponse({ request, response }) {
      ...
      // Intercept 402 (insufficient credits) and show a toast
      if (response.status === 402) {
        await handle402Response(response.clone(), ({ message, action }) =>
          toast.error(message, {
            action: action
              ? { label: action.label, onClick: () => window.open(action.href, '_blank') }
              : undefined,
          })
        )
        return response
...
export async function handleFetchError(response: unknown): Promise<ResponseError> {
  let resJson: any = {}
  ...
  if (response instanceof Response) {
    // Intercept 402 (insufficient credits) and show a toast before building the error
    if (response.status === 402) {
      await handle402Response(response.clone(), ({ message, action }) =>
        toast.error(message, {
          action: action
      ...
      // Return ResponseError with empty message so React Query onError
      // handlers don't double-surface the 402 (the toast already fired).
      // Callers can check err.code === 402 if they need to branch.
      return new ResponseError('', 402)
    }
