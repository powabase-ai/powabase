...
> = {}) => {
  ...
  return useMutation<ProjectDeleteData, ResponseError, ProjectDeleteVariables>({
    ...
    async onSuccess(data, variables, context) {
      // Run user onSuccess first so callers can navigate away from the
      // deleted project's route before we touch the cache. Cache changes
      // trigger observer re-renders; if any still-mounted component reads
      // a field on the deleted project's detail, it will crash.
      await onSuccess?.(data, variables, context)

      // Remove (not invalidate) the detail query: the project is gone, so
      // there's nothing to refetch. invalidateQueries would trigger a 404.
      queryClient.removeQueries({ queryKey: projectKeys.detail(data.ref) })
