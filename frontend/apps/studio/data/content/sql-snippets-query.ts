...
export async function getSqlSnippets(
  ...
  // DEBUG: trace what openapi-fetch returns for content endpoint
  console.log('[DEBUG sql-snippets] raw response:', { data, error, dataType: typeof data })
  if (data) {
    console.log('[DEBUG sql-snippets] data.cursor:', data.cursor, 'data.data:', data.data, 'typeof data.data:', typeof data.data)
  }

  if (error) {
    console.log('[DEBUG sql-snippets] throwing error:', error)
    throw error
  }

  const contents = (data?.data ?? []) as unknown as SqlSnippet[]
  console.log('[DEBUG sql-snippets] returning contents:', contents, 'length:', contents.length)
