  ...
  async redirects() {
    return [
      ...
      ...(process.env.MAINTENANCE_MODE === 'true'
        ? [
            {
              source: '/((?!maintenance|img|email-templates).*)', // Redirect all paths except /maintenance, /img, and /email-templates (GoTrue fetches these and silently falls back to its built-in default if it gets the maintenance HTML instead — the default has the phishing-shaped redirect_to URL we're trying to remove).
