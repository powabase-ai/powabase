...
export async function GET() {
  // Skip for non-Supabase-cloud deployments. The happy-path branch below
  // returns IncidentInfo[]; the client (data/platform/incident-status-query.ts)
  // calls lodash.partition on the response, which treats an object as
  // array-like and yields a spurious "1 other issue" banner. Return [].
  if (!IS_PLATFORM || !process.env.NEXT_PUBLIC_INCIDENT_IO_API_URL) {
