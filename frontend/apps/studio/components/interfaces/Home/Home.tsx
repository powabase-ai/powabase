...
export const Home = () => {
  ...
  // Powabase compute-tier feature (separate from the IS_PLATFORM-gated Supabase
  // compute add-on above). Gated on the single per-org billing-UI switch (same
  // flag as the B1 plan picker), resolved for this project's org.
  const showComputeTier = useIsBillingUiEnabled(organization)
  const resume = useProjectResumeMutation()
  // `compute_size_id` is surfaced by the B2 BE project-detail (Task 3.3) but the
  // generated OpenAPI types haven't been regenerated yet — read it narrowly.
                ...
                {showComputeTier && (
                  <>
                    ...
                    <Button asChild type="default" size="tiny">
                      <Link href={`/project/${project?.ref}/infrastructure`}>Manage compute</Link>
                    </Button>
                  </>
                )}
              </div>
