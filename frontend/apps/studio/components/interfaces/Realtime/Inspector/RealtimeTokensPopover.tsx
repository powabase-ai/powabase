  ...
  useEffect(() => {
    const triggerUpdateTokenBearer = async () => {
      ...
      if (
        ...
        // Self-hosted backends without a temp-key endpoint can return `{}`
        // (issue #212). Fall back to the project's anon key so RealtimeClient
        // gets a non-empty `params.apikey` instead of throwing at connect time.
        if (!token) {
          token = publishableKey?.api_key ?? anonKey?.api_key
        }
      }
