...
describe('parseCronJobCommand', () => {
  ...
  it('should return an http_request config when the command posts to a supabase.co functions URL (no Deno runtime in Agentic Platform — no dedicated edge function type)', () => {
    ...
    expect(parseCronJobCommand(command, 'random_project_ref')).toStrictEqual({
      endpoint: 'https://random_project_ref.supabase.co/functions/v1/_',
      method: 'POST',
      httpHeaders: [
        {
          name: 'Authorization',
  ...
  it('should return an http_request config when the body is missing', () => {
  ...
  it("should return an HTTP request config when there's a query parameter or hash in the URL", () => {
