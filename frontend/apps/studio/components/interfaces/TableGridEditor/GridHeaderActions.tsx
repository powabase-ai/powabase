...
export const GridHeaderActions = ({ table, isRefetching }: GridHeaderActionsProps) => {
  ...
  const showApiDocsButton = table.schema === 'public'
  ...
  const { hasLint: viewHasLints } = getEntityLintDetails(
    table.name,
  ...
  const { hasLint: materializedViewHasLints } = getEntityLintDetails(
    table.name,
    ...
    <div className="sb-grid-header__inner">
      {showHeaderActions && (
        <div className="flex items-center gap-x-2">
          ...
          {showApiDocsButton && (
            <APIDocsButton source="table_editor" />
          )}
