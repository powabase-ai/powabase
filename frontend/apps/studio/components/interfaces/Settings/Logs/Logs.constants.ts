...
group by
  ...
  {
    label: 'Metadata IP',
    description: 'List all IP addresses that used the Powabase API',
    mode: 'custom',
    searchString: `select
...
`,
  ...
  },
  {
    ...
    description: 'List all ISO 3166-1 alpha-2 country codes that used the Powabase API',
    mode: 'custom',
    searchString: `select
  cf.country,
  count(*) as count
...
`,
  ...
  },
  {
    ...
    mode: 'custom',
    description: 'List all Powabase API requests that are slow',
    searchString: `select
  cast(timestamp as datetime) as timestamp,
  event_message,
  r.origin_time
...
`,
  ...
  },
  {
    ...
    description: 'List all Powabase API requests that responded witha 5XX status code',
    mode: 'custom',
    searchString: `select
  cast(timestamp as datetime) as timestamp,
  event_message,
...
`,
  ...
  {
    label: 'Top Paths',
    description: 'List the most requested Powabase API paths',
    mode: 'custom',
    searchString: `select
  r.path as path,
  r.search as params,
...
// [Powabase] The Logs Explorer routes this gated (/project/[ref]/logs/**) were removed;
// kept as an empty allowlist so the (dead, unreferenced) preview-panel consumers below still
// type-check without re-deriving route strings for pages that no longer exist.
export const LOG_ROUTES_WITH_REPLICA_SUPPORT: string[] = []
