...
export const SubscriptionPlanUpdateDialog = ({
  ...
  const onSuccessfulPlanChange = () => {
    ...
    // Google Ads paid-conversion. Only on an actual upgrade ('upgrade' covers
    // Free→Pro, Pro→Team, etc.) — downgrades and same-plan ('none') changes are
    // not paid conversions. Reached from both the no-extra-confirmation path and
    // the post-Stripe-payment-intent path, so it covers every successful
    // upgrade. Fires regardless of consent: under Consent Mode v2 (see
    // google-analytics-tag.tsx) it's sent cookielessly and modeled when consent
    // isn't granted. No-op until the Ads id + label env vars are set and gtag.js
    // has loaded.
    if (changeType === 'upgrade' && GOOGLE_ADS_ID && GOOGLE_ADS_PAID_LABEL) {
