...
export const SupportFormV2 = ({ form, initialError, state, dispatch }: SupportFormV2Props) => {
  ...
  const onSubmit: SubmitHandler<SupportFormValues> = async (formValues) => {
    ...
    // Strip the attachDashboardLogs key so it doesn't leak into the payload
    // along with the rest of the form values.
    const { attachDashboardLogs: _, ...values } = formValues

    // Attachments + dashboard-log upload disabled: Powabase has no support
    // storage backend yet. Re-enable when Storage lands in the platform stack.
    ...
    <Form_Shadcn_ {...form}>
      <form id="support-form" className="flex flex-col gap-y-6">
        ...
        <div className="px-6 flex flex-col gap-y-8">
          ...
          {/* AttachmentUpload + DashboardLogsToggle hidden: Powabase has no
              support-attachments storage backend yet. Re-enable when the
              Storage service is added to the platform Supabase stack. */}
        </div>
