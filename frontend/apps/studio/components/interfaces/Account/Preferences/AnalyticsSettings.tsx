...
export const AnalyticsSettings = () => {
  const { hasAccepted, acceptAll, denyAll, categories, source } = useConsentState()
  // 'bridge' = consent is managed via the shared cross-domain cookie (no CMP):
  // acceptAll/denyAll write that cookie, so the toggle works without categories.
  // categories !== null covers the CMP path. Only a genuine CMP-load failure
  // (source null, no categories) lacks a working control.
  const hasLoaded = categories !== null || source === 'bridge'
