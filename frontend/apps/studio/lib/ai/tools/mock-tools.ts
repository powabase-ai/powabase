...
const getLogsInputSchema = z.object({
  ...
  source: z.enum(['postgres', 'auth', 'storage']).optional(),
...
const MOCK_ADVISORIES_DATA = [
  ...
  },
  {
    ...
    message:
      'Query on table audit_log_entries exceeded 3s average execution time over the last hour.',
    remediationUrl: '',
  },
]
...
function createMockedStudioTools() {
  ...
  return Object.fromEntries(
    Object.entries(studioTools).map(([name, baseTool]) => {
      ...
      const statusMessage =
        name === 'execute_sql' ? 'SQL execution mocked successfully.' : 'Tool call mocked successfully.'
