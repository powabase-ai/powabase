 ...
 *
 * @param baseUrl - The base URL for API requests (e.g., https://app.powabase.ai)
    ...
    execute: async () => {
      ...
      try {
        ...
        return {
          ...
          message: `There ${incidents.length === 1 ? 'is' : 'are'} ${incidents.length} active incident${incidents.length === 1 ? '' : 's'} on Powabase infrastructure. If the user's issue appears related, inform them about the ongoing incident(s).`,
        }
      } catch (error) {
