...
import { hasConsented, useConsentState, useIsLoggedIn, useUser } from 'common'
import { useRouter } from 'next/router'
import { createContext, PropsWithChildren, useContext, useEffect, useMemo } from 'react'
import { toast } from 'sonner'

import { useSignOut } from './auth'
import { GOOGLE_ADS_ID, GOOGLE_ADS_SIGNUP_LABEL, REDDIT_PIXEL_ID } from './constants'
...
export const ProfileProvider = ({ children }: PropsWithChildren<{}>) => {
  ...
  const { mutate: createProfile, isPending: isCreatingProfile } = useProfileCreateMutation({
    onSuccess: () => {
      ...
      if (user) {
        ...
        // GA4 sign_up. gtag.js now loads regardless of consent (Consent Mode v2,
        // see google-analytics-tag.tsx), so this fires for everyone — cookieless
        // and modeled when consent isn't granted. Dropped (not queued) only in
        // the brief window before gtag.js has finished loading.
        window.gtag?.('event', 'sign_up', {
          method: user.app_metadata?.provider ?? 'email',
        })

        // Google Ads sign-up conversion. Fires regardless of consent: under
        // Consent Mode v2 (see google-analytics-tag.tsx) the event is sent
        // cookielessly and modeled by Google when consent isn't granted, which
        // captures sign-ups from users who never passed through the consent
        // banner. No-op (dropped, not queued) until the Ads id + label env vars
        // are set and gtag.js has loaded.
        ...
        // Reddit pixel hard-gates on consent — deliberately, unlike the Google
        // tags above which now fire always under Consent Mode (Reddit has no
        // Consent Mode equivalent). Don't "align" this. It also re-checks
        // consent because pixel.js stays loaded after the <Script> unmounts, so
        // accept → deny in-tab would otherwise keep firing.
        if (hasConsented()) {
  ...
  // Re-emit on logout (deps go undefined) so the previous user's pixel binding clears.
  useEffect(() => {
    if (!REDDIT_PIXEL_ID || !hasAccepted) return
    const email = user?.email ?? profile?.primary_email
    const externalId = profile?.gotrue_id
    window.rdt?.('init', REDDIT_PIXEL_ID, {
      email: email ?? undefined,
      externalId: externalId ?? undefined,
    })
  }, [hasAccepted, user?.email, profile?.primary_email, profile?.gotrue_id])
